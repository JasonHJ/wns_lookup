  ...
  try {
    ...
    if (status === '2') {
      res.json({
        ...
        value: web3.fromWei(deedContract.at(contractInstance.entries(web3.sha3(domain))[1]).value()),
        highestBid: web3.fromWei(contractInstance.entries(web3.sha3(domain))[4])
      })
    }
    if (status === '3') {
      res.json({
